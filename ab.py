...
for i in range(10):
    ...
    aug_n = 1
    aug_m = [5]
    aug_p = [(np.array(list(da.ranges.keys()))=='Window Warping').astype(int)]
    ...
    aug_results.to_excel(datasets[0]+"/seed_"+str(seed)+"_aug_results3.xlsx")
    ...
    aug_results = pd.concat(results)
    aug_results['seed']  = seed
    aug_results.to_excel(datasets[0]+"/seed_"+str(seed)+"_augmix_results3.xlsx")
